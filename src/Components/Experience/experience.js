...
export default function Experience() {
    ...
    const handleExperienceDate = (startY, endY, startMo, endMo) => {
        if(!startY) return '-';
        return !startMo || !endMo ? `${startY} - ${endY ? endY : 'Present'}` :
    ...
    return (
        <section id='experience' className="experience-container" data-aos="fade-up" data-aos-duration="800" data-aos-once={true}>
            <div className="experience inner-wrapper">
                <h1>{experience.title}</h1>
                <div>
                    <ul className='work-place-container'>
                        {expList.map((exp, index) => (
                            <li className={`experience-item ${index === selectedTab && 'active'}`} key={Math.random() + exp.company}>
