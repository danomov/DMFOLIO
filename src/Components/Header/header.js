...
export default function Header() {
  ...
  const handleLoadNavigationMenu = useCallback(() => {
      ...
      if(navMenu && isMobile) setTimeout(() => navMenu.style.display = 'initial', 500);
  }, [isMobile])
  ...
  return (
    <header>
      <Link to="/" id="logo" />
      <nav className={`${isMobile ? 'mobile-menu' : ''} ${isMenuOpen && isMobile ? 'open' : ''}`}>
        <ul>
      ...
      {isMobile && <button className='menu-button' onClick={handleToggleMenu}/>}
    </header>
  );
}
